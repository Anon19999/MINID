...
def train_model():
    ...
    training_tensor_datasets, testing_tensor_datasets = dataset.get_training_testing_tensor_datasets(
        tensor_dataset=dataset.create_tensor_dataset(
            dataset.load_tensor_from_mat_file(path='halftone_reshape.mat', key='halftone_reshape'),
...
def save_model_weights_and_biases():
    ...
    utils.save_model_weights_and_biases(model=forward_network, path=os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'MILP', 'Data', '44ink_net_50_50_w_b.mat'), key='w_numpy')
