...
def train_model():
    ...
    training_tensor_datasets, testing_tensor_datasets = dataset.get_training_testing_tensor_datasets(
        tensor_dataset=dataset.create_tensor_dataset(
            dataset.load_tensor_from_mat_file(path='dataset_halftone.mat', key='dataset_halftone'),
            dataset.load_tensor_from_mat_file(path='dataset_approx_spec.mat', key='dataset_approx_spec')
        ),
...
def save_model_weights_and_biases():
    ...
    utils.save_model_weights_and_biases(model=forward_network, path=os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'MILP', 'Data', '8ink_net_4lay_150_w_b.mat'), key='w_numpy')
