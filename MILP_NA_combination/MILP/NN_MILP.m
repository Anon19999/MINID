...
logPathTemplate = 'Data/sample_%i_4lay_150.log';
timeLim = input("Please enter the time limit for the optimizer (default=5000s):\n");
if isempty(timeLim)
    timeLim = 5000;
end
...
for i = 101:100:401
    ...
    logPath = sprintf(logPathTemplate,i);
    

    
    delete(logPath) % Delete existing log file

    % Define the objective
    objective =   sum(sum(Z));
    options = sdpsettings('solver','gurobi', 'gurobi.Threads', 0,  'gurobi.MIPFocus', 0,'gurobi.LogFile', logPath, 'gurobi.Timelimit', timeLim);
    tic
